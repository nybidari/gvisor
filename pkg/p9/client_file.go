...
func (c *clientFile) SetXattr(name, value string, size uint64, flags uint32) error {
	if atomic.LoadUint32(&c.closed) != 0 {
		return syscall.EBADF
	}
	if !versionSupportsGetSetXattr(c.client.version) {
	...
	return c.client.sendRecv(&Tsetxattr{FID: c.fid, Name: name, Value: value, Size: size, Flags: flags}, &Rsetxattr{})
}
