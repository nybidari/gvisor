...
func newSubtasks(creds *auth.Credentials, inoGen InoGenerator, task *kernel.Task, pidns *kernel.PIDNamespace) (*subtasksInode, *kernfs.Dentry) {
	inode := &subtasksInode{
		task:   task,
		pidns:  pidns,
		inoGen: inoGen,
	}
	inode.InodeAttrs.Init(creds, inoGen.NextIno(), linux.ModeDirectory|dirPerm)
...
func (i *subtasksInode) Lookup(ctx context.Context, name string) (*vfs.Dentry, error) {
	...
	_, subTaskDentry := newTaskInode(i.inoGen, subTask, i.pidns, false)
