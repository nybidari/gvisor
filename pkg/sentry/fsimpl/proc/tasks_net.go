...
func newNetDir(root *auth.Credentials, inoGen InoGenerator, k *kernel.Kernel) *kernfs.Dentry {
	...
	if stack := k.NetworkStack(); stack != nil {
		contents = map[string]*kernfs.Dentry{
			"dev":  kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netDevData{stack: stack}),
			"snmp": kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netSnmpData{stack: stack}),

			// The following files are simple stubs until they are implemented in
			// netstack, if the file contains a header the stub is just the header
			// otherwise it is an empty file.
			"arp":       kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "IP address       HW type     Flags       HW address            Mask     Device"}),
			"netlink":   kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "sk       Eth Pid    Groups   Rmem     Wmem     Dump     Locks     Drops     Inode"}),
			"netstat":   kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "TcpExt: SyncookiesSent SyncookiesRecv SyncookiesFailed EmbryonicRsts PruneCalled RcvPruned OfoPruned OutOfWindowIcmps LockDroppedIcmps ArpFilter TW TWRecycled TWKilled PAWSPassive PAWSActive PAWSEstab DelayedACKs DelayedACKLocked DelayedACKLost ListenOverflows ListenDrops TCPPrequeued TCPDirectCopyFromBacklog TCPDirectCopyFromPrequeue TCPPrequeueDropped TCPHPHits TCPHPHitsToUser TCPPureAcks TCPHPAcks TCPRenoRecovery TCPSackRecovery TCPSACKReneging TCPFACKReorder TCPSACKReorder TCPRenoReorder TCPTSReorder TCPFullUndo TCPPartialUndo TCPDSACKUndo TCPLossUndo TCPLostRetransmit TCPRenoFailures TCPSackFailures TCPLossFailures TCPFastRetrans TCPForwardRetrans TCPSlowStartRetrans TCPTimeouts TCPLossProbes TCPLossProbeRecovery TCPRenoRecoveryFail TCPSackRecoveryFail TCPSchedulerFailed TCPRcvCollapsed TCPDSACKOldSent TCPDSACKOfoSent TCPDSACKRecv TCPDSACKOfoRecv TCPAbortOnData TCPAbortOnClose TCPAbortOnMemory TCPAbortOnTimeout TCPAbortOnLinger TCPAbortFailed TCPMemoryPressures TCPSACKDiscard TCPDSACKIgnoredOld TCPDSACKIgnoredNoUndo TCPSpuriousRTOs TCPMD5NotFound TCPMD5Unexpected TCPMD5Failure TCPSackShifted TCPSackMerged TCPSackShiftFallback TCPBacklogDrop TCPMinTTLDrop TCPDeferAcceptDrop IPReversePathFilter TCPTimeWaitOverflow TCPReqQFullDoCookies TCPReqQFullDrop TCPRetransFail TCPRcvCoalesce TCPOFOQueue TCPOFODrop TCPOFOMerge TCPChallengeACK TCPSYNChallenge TCPFastOpenActive TCPFastOpenActiveFail TCPFastOpenPassive TCPFastOpenPassiveFail TCPFastOpenListenOverflow TCPFastOpenCookieReqd TCPSpuriousRtxHostQueues BusyPollRxPackets TCPAutoCorking TCPFromZeroWindowAdv TCPToZeroWindowAdv TCPWantZeroWindowAdv TCPSynRetrans TCPOrigDataSent TCPHystartTrainDetect TCPHystartTrainCwnd TCPHystartDelayDetect TCPHystartDelayCwnd TCPACKSkippedSynRecv TCPACKSkippedPAWS TCPACKSkippedSeq TCPACKSkippedFinWait2 TCPACKSkippedTimeWait TCPACKSkippedChallenge TCPWinProbe TCPKeepAlive TCPMTUPFail TCPMTUPSuccess"}),
			"packet":    kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "sk       RefCnt Type Proto  Iface R Rmem   User   Inode"}),
			"protocols": kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "protocol  size sockets  memory press maxhdr  slab module     cl co di ac io in de sh ss gs se re sp bi br ha uh gp em"}),
			...
			"psched": kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: fmt.Sprintf("%08x %08x %08x %08x\n", uint64(time.Microsecond/time.Nanosecond), 64, 1000000, uint64(time.Second/time.Nanosecond))}),
			"ptype":  kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "Type Device      Function"}),
			"route":  kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netRouteData{stack: stack}),
			"tcp":    kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netTCPData{kernel: k}),
			"udp":    kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netUDPData{kernel: k}),
			"unix":   kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netUnixData{kernel: k}),
		}

		if stack.SupportsIPv6() {
			contents["if_inet6"] = kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &ifinet6{stack: stack})
			contents["ipv6_route"] = kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{})
			contents["tcp6"] = kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &netTCP6Data{kernel: k})
			contents["udp6"] = kernfs.NewDynamicBytesFile(root, inoGen.NextIno(), filePerm, &vfs.StaticData{Data: "  sl  local_address                         remote_address                        st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode"})
		}
	}

	return kernfs.NewStaticDir(root, inoGen.NextIno(), dirPerm, contents)
}
...
var _ vfs.DynamicBytesSource = (*netDevData)(nil)
...
var _ vfs.DynamicBytesSource = (*netUnixData)(nil)
...
func commonGenerateTCP(ctx context.Context, buf *bytes.Buffer, k *kernel.Kernel, family int) error {
	...
	for _, se := range k.ListSockets() {
		...
		if family, stype, _ := sops.Type(); !(family == family && stype == linux.SOCK_STREAM) {
			s.DecRef()
			// Not tcp4 sockets.
			continue
		}
...
var _ vfs.DynamicBytesSource = (*netTCPData)(nil)
...
var _ vfs.DynamicBytesSource = (*netTCP6Data)(nil)
...
var _ vfs.DynamicBytesSource = (*netUDPData)(nil)
...
var _ vfs.DynamicBytesSource = (*netSnmpData)(nil)
...
var _ vfs.DynamicBytesSource = (*netRouteData)(nil)
