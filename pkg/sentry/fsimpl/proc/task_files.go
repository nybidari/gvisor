...
// getTaskMM returns t's MemoryManager. If getTaskMM succeeds, the
...
var _ vfs.DynamicBytesSource = (*cmdlineData)(nil)
...
func newComm(creds *auth.Credentials, ino uint64, perm linux.FileMode, task *kernel.Task) *kernfs.Dentry {
	inode := &commInode{task: task}
	inode.DynamicBytesFile.Init(creds, ino, &commData{task: task}, perm)
...
var _ vfs.DynamicBytesSource = (*commData)(nil)
...
// namespaceData implements vfs.DynamicBytesSource for /proc/[pid]/ns/[namespace].
//
// +stateify savable
...
var _ vfs.DynamicBytesSource = (*namespaceData)(nil)

// Generate implements vfs.DynamicBytesSource.Generate.
//
// Namespace symlinks should contain the namespace name and the inode number
// for the namespace instance, so for example user:[123456]. We currently
// fake the inode number by sticking the symlink inode in its place.
func (d *namespaceData) Generate(ctx context.Context, buf *bytes.Buffer) error {
	fmt.Fprintf(buf, "%s:[%d]", d.ns, d.ino)
	return nil
}
...
var _ vfs.DynamicBytesSource = (*idMapData)(nil)
...
func (md *mapsCommon) mm() *mm.MemoryManager {
	...
	md.task.WithMuLocked(func(t *kernel.Task) {
...
// selfData provides data for /proc/self symlink.
//
// +stateify savable
...
func (d *selfData) Generate(ctx context.Context, buf *bytes.Buffer) error {
	t := kernel.TaskFromContext(ctx)
	if t == nil {
		// Who is reading this link?
		return syserror.EINVAL
	}
	tgid := d.pidns.IDOfThreadGroup(t.ThreadGroup())
	if tgid == 0 {
		return syserror.ENOENT
	}
	buf.WriteString(strconv.FormatUint(uint64(tgid), 10))
	return nil
}

// theadSelfData provides data for /proc/thread-self symlink.
//
// +stateify savable
type theadSelfData struct {
	pidns *kernel.PIDNamespace
}

// Generate implements vfs.DynamicBytesSource.
func (d *theadSelfData) Generate(ctx context.Context, buf *bytes.Buffer) error {
	t := kernel.TaskFromContext(ctx)
	if t == nil {
		// Who is reading this link?
		return syserror.EINVAL
	...
	tgid := d.pidns.IDOfThreadGroup(t.ThreadGroup())
	tid := d.pidns.IDOfTask(t)
	if tid == 0 || tgid == 0 {
		return syserror.ENOENT
	}
	fmt.Fprintf(buf, "%d/task/%d", tgid, tid)
	return nil
}
