...
type InodeOperations interface {
	...
	// ENODATA.
	//
	// Note that size is a hint that implementations are free to ignore. If a
	// value that is larger or smaller is returned, the size handling will be
	// done at the syscall layer.
	...
	SetXattr(ctx context.Context, inode *Inode, name, value string, size uint64, flags uint32) error
