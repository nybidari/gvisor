...
func (i *Inode) SetXattr(ctx context.Context, name, value string, size uint64, flags uint32) error {
	if i.overlay != nil {
		return overlaySetxattr(ctx, i.overlay, name, value, size, flags)
	}
	return i.InodeOperations.SetXattr(ctx, i, name, value, size, flags)
}
