...
var regexTodo = regexp.MustCompile(`(\/\/|#)\s*TODO\(([a-zA-Z0-9.\/]+)\):\s*(.+)`)
	...
	for _, fi := range fis {
		...
		case fi.Mode().IsRegular():
			...
			for scanner.Scan() {
				lineno++
				line := scanner.Text()
				if matches := regexTodo.FindStringSubmatch(line); matches != nil {
					if len(matches) != 4 {
...
func (r *Reviver) processTodo(todo *Todo) (bool, error) {
	...
	// fmt.Printf("No handler found for %q, locations: %v", todo.Issue, todo.Locations)
	return false, nil
}
