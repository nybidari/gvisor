...
class GCloudProducer(machine_producer.MachineProducer):
  ...
  Attributes:
    ...
    image project: image project as a string.
  ...
  def __init__(self,
    ...
    self.ssh_key_path = ssh_key_path.format(user=DEFAULT_USER)
    self.image = image
    self.image_project = image_project
    self.zone = zone
    self.ssh_user = ssh_user if ssh_user else DEFAULT_USER
    self.ssh_password = ssh_password if ssh_password else ""
    self.mock = mock
  ...
  def _run_command(self,
                   cmd: List[str],
                   detach=False) -> [None, subprocess.CompletedProcess]:
      ...
      if self.mock:
        ...
        self.mock.record(
            subprocess.CompletedProcess(
                returncode=p.returncode, stdout=p.stdout, args=p.args))
      return
