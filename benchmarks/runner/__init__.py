...
def run_local(ctx, limit: float, **kwargs):
  ...
  run(ctx, machine_producer.LocalMachineProducer(max_machines=limit), **kwargs)
...
@click.pass_context
def run_gcp(ctx, project: str, ssh_key: str, image: str, image_project: str,
            zone: str, ssh_user: str, ssh_password: str, **kwargs):
  """Runs all benchmarks on GCP instances.

  Args:
    ctx: context to pass to child run method.
    project: GCP project ID
    ssh_key: path to a valid ssh_key. See docs on what a valid ssh key is.
    image: name of the image to use to build images
    image_project: project under which 'image' is stored. If left unset, is set
    to project.
    zone: a valid GCP zone
    ssh_user: user for ssh_key
    ssh_password: password for the ssh_key
    **kwargs: to pass to run
  """
