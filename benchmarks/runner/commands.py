...
class LocalCommand(RunCommand):
  ...
  def __init__(self, *args, **kwargs):
    super().__init__(*args, **kwargs)
    self.params.append(
        click.core.Option(("--limit",),
                          default=float("inf"),
                          help="Limit the number of machines provided to run."))
...
class GCPCommand(RunCommand):
  ...
  def __init__(self, *args, **kwargs):
    ...
    ssh_key_path = click.core.Option(
        ("--ssh-key",),
        help="Path to a valid ssh key to use. See README on generating a valid ssh key.",
    ...
    ssh_password = click.core.Option(("--ssh-password",),
                                     ...
                                     default="")
    self.params.extend([
        project, ssh_key_path, image, image_project, zone, ssh_user,
        ssh_password
    ])
